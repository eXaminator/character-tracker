...
test('Users can update their password', async ({ login, page }) => {
    ...
    await fieldset.getByRole('textbox', { name: /^current password/i }).fill(oldPassword);
    await fieldset.getByRole('textbox', { name: /^new password/i }).fill(newPassword);

    await page.getByRole('button', { name: /^save/i }).click();

    await expect(page).toHaveURL(`/users/${user.username}`);

    expect(await verifyLogin(user.username, oldPassword), 'Old password still works').toEqual(null);
    expect(await verifyLogin(user.username, newPassword), 'New password does not work').toEqual({ id: user.id });
});
...
test('Users can update their profile photo', async ({ login, page }) => {
    ...
    await page.getByLabel(/change/i).setInputFiles('./tests/fixtures/test-profile.jpg');
